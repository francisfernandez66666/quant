<template>
  <div class="watchlist-page">
    ...
    <div class="eval-table" v-if="stocks.length">
      <div class="table-scroll">
        <div class="table-inner">
          <div class="ev-header">
            <span class="ev-code sortable" @click.stop="setSort('code')">代码{{ sortArrow('code') }}</span>
            <span class="ev-name sortable" @click.stop="setSort('name')">名称{{ sortArrow('name') }}</span>
            <span class="ev-price sortable" @click.stop="setSort('price')">现价{{ sortArrow('price') }}</span>
            <span class="ev-chg sortable" @click.stop="setSort('change_pct')">涨跌{{ sortArrow('change_pct') }}</span>
            <span class="ev-scr sortable" @click.stop="setSort('n_score')" title="N≥60">N</span>
            <span class="ev-scr sortable" @click.stop="setSort('dragon_score')" title="龙≥70">龙</span>
            <span class="ev-scr sortable" @click.stop="setSort('db_score')" title="凸≥70">凸</span>
            <span class="ev-scr sortable" @click.stop="setSort('dr_score')" title="回≥60">回</span>
            <span class="ev-scr sortable" @click.stop="setSort('m_score')" title="量≥50">量</span>
          </div>
          <div v-for="e in sortedEvals" :key="e.code" :class="rowClass(e)" @click="tapRow(e)">
    ...
    <div class="action-sheet" v-if="actionTarget">
      ...
      <button class="sheet-btn sheet-danger" @click="remove(actionTarget.code); actionTarget = null">删除自选</button>
...
function tapRow(e) { actionTarget.value = e }
...
async function load() {
  try {
    ...
    // 兜底：快照也查不到的，放个占位，等下次轮询补名
    for (const c of codes) {
      if (!stocks.value.find(s => s.code === c)) {
        const fallback = { code: c, name: c, price: 0, change_pct: 0,
        ...
        // 异步从快照补名称
        if (snap && snap.length) {
          const found = snap.find(s => s.code === c)
          if (found && found.name) fallback.name = found.name
        }
        stocks.value.push(fallback)
      }
    }
...
.add-row input {
  ...
  background: #0f0f23; color: #e0e0e0; font-size: 16px; width: 160px; outline: none;
}
...
.feedback { font-size: 16px; padding: 4px 10px; border-radius: 4px; white-space: nowrap; }
.feedback.ok { color: #4caf50; }
.feedback.err { color: #FF4D4F; }
.eval-table { font-size: 16px; background: #1a1a2e; border-radius: 10px; overflow: hidden; }
.table-scroll { overflow-x: auto; }
.table-inner { min-width: 660px; }
.ev-header, .table-inner > div {
  display: flex; align-items: center; min-height: 44px; padding: 4px 12px; gap: 0;
  border-bottom: 1px solid #1a1a26;
}
.ev-header { background: #2a2a3e; color: #888; font-weight: 600; }
...
.ev-score, .ev-scr { flex: 1; text-align: center; color: #555; font-weight: 600; }
...
.ev-table .table-inner > div { cursor: pointer; }
.ev-table .table-inner > div:active { background: rgba(255,255,255,0.05); }
...
.add-bar {
  position: fixed; bottom: 68px; left: 0; right: 0; z-index: 110;
...
.add-bar input:focus { border-color: #FF4D4F; }
.add-bar .btn-add {
  padding: 12px 22px; border-radius: 8px; border: none;
  background: #FF4D4F; color: #fff; font-size: 16px; cursor: pointer; flex-shrink: 0;
}
.add-bar .btn-add:disabled { opacity: 0.5; }
.add-bar .feedback { font-size: 16px; padding: 4px 10px; border-radius: 4px; white-space: nowrap; }
.add-bar .feedback.ok { color: #4caf50; }
.add-bar .feedback.err { color: #FF4D4F; }
.watchlist-page { padding-bottom: 72px; }
.empty { text-align: center; padding: 40px; color: #555; font-size: 16px; }
.legend {
  margin-top: 16px; padding: 10px 16px; font-size: 16px; color: #666;
  background: #1a1a2e; border-radius: 8px; display: flex; align-items: center; gap: 12px; flex-wrap: wrap;
}
